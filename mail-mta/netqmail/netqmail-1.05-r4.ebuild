...
# $Header: /var/cvsroot/gentoo-x86/mail-mta/netqmail/netqmail-1.05-r4.ebuild,v 1.16 2006/11/23 20:11:54 blubb Exp $
...
IUSE="gencertdaily highvolume mailwrapper noauthcram qmail-spp ssl vanilla virusscan"
RESTRICT="test"
...
src_unpack() {
	...
	if use virusscan; then
		epatch ${FILESDIR}/qmail-smtpd-viruscan-1.3+netqmail.patch
	fi
...
src_install() {
	...
	if use virusscan; then
		einfo "Installing base signatures file"
		insinto /var/qmail/control
		insopts -o root -g root -m 644
		doins ${FILESDIR}/signatures
	fi
